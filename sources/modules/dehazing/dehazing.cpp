...
void dehazing::ImageHazeRemoval(Mat& mSrc, Mat& mDst)
{
	...
	MakeExpLUT();
	//PfmTest_GetFunCostTime(MakeExpLUT(), "MakeExpLUT");

	//GuideLUTMaker();
	//PfmTest_GetFunCostTime(GuideLUTMaker(), "GuideLUTMaker");

	GammaLUTMaker(0.7f); 
	//PfmTest_GetFunCostTime(GammaLUTMaker(0.7f), "GammaLUTMaker");

	AirlightEstimation(mSrc);
	//PfmTest_GetFunCostTime(AirlightEstimation(mSrc), "AirlightEstimation");

	/* iplimage to int */
	//IplImageToIntColor(mSrc);
	//PfmTest_GetFunCostTime(IplImageToIntColor(mSrc), "IplImageToIntColor");
	...
	//PfmTest_GetFunCostTime(TransmissionEstimationColor(m_pnRImg, m_pnGImg, m_pnBImg, m_pfTransmission, m_pnRImg, m_pnGImg, m_pnBImg, m_pfTransmission, 0, m_nWid, m_nHei), "TransmissionEstimationColor");
    TransmissionEstimationColor(mSrc, m_pfTransmission, m_pnRImg, m_pnGImg, m_pnBImg, m_pfTransmission, 0, m_nWid, m_nHei);
    //PfmTest_GetFunCostTime( TransmissionEstimationColor(mSrc, m_pfTransmission, m_pnRImg, m_pnGImg, m_pnBImg, m_pfTransmission, 0, m_nWid, m_nHei), "myTransmissionEstimationColor");


	//GuidedFilter(m_nWid, m_nHei, 0.001);
	//PfmTest_GetFunCostTime(GuidedFilter(m_nWid, m_nHei, (float)0.001), "GuidedFilter");
    myGuidedFilter( mSrc, m_nGBlockSize, (float)0.001 );
    //PfmTest_GetFunCostTime(myGuidedFilter( mSrc, m_nGBlockSize, (float)0.001), "myGuidedFilter");
	...
	*/
	...
	//PfmTest_GetFunCostTime(RestoreImage(mSrc, mDst), "RestoreImage");
}
