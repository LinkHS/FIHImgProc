...
void dehazing::TransmissionEstimationColor(Mat &mSrc, float *pfTransmission,int *pnImageRP, int *pnImageGP, int *pnImageBP, float *pfTransmissionP,int nFrame, int nWid, int nHei)
{
	int nX, nY, nXstep, nYstep;
	float fTrans;
...
//TODO
	...
	else
    {
    	for(nY=0; nY<nHei; nY+=m_nTBlockSize)
    	{
    		for(nX=0; nX<nWid; nX+=m_nTBlockSize)
    		{
    			fTrans = myNFTrsEstimationColor(mSrc, nX, nY, nWid, nHei);
...
float dehazing::myNFTrsEstimationColor(Mat &mSrc, int nStartX, int nStartY, int nWid, int nHei)
{
	int nCounter;	
	int nX, nY;		
	int nEndX;
	...
	for(nCounter=0; nCounter<7; nCounter++)
	{
		...
		for(nY=nStartY; nY<nEndY; nY++)
		{
			...
			uchar* pSrc = (uchar*)(mSrc.data + mSrc.step.p[0] * nY);

			//for(nX=nStartX; nX<nEndX; nX++)
			for(nX=nStartX*3; nX<nEndX*3; nX+=3)
			{
				/* (I-A)/t + A --> ((I-A)*k*128 + A*128)/128 */
				nOutB = ((pSrc[nX+0] - m_anAirlight[0])*nTrans + 128*m_anAirlight[0]) >> 7;
				nOutG = ((pSrc[nX+1] - m_anAirlight[1])*nTrans + 128*m_anAirlight[1]) >> 7;
				nOutR = ((pSrc[nX+2] - m_anAirlight[2])*nTrans + 128*m_anAirlight[2]) >> 7;
...
//  	if(temp == 1)
//  	{
//  		printf("fMean=%f,fCost=%f,nSumofOuts=%d,nNumberofPixels=%d,nSumofSLoss=%d,nSumofSquaredOuts=%d\n",\
//  				 fMean, fCost, nSumofOuts, nNumberofPixels, nSumofSLoss, nSumofSquaredOuts);
//  	}
