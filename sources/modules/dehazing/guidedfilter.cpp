...
void dehazing::myGuidedFilter(Mat &mGuide, int r, float eps)
{
    Mat mGray;
    cvtColor( mGuide, mGray, CV_BGR2GRAY );

    Mat mSrc( mGuide.rows, mGuide.cols, CV_32FC1, m_pfTransmission );
    Mat mDst( mGuide.rows, mGuide.cols, CV_32FC1, m_pfTransmissionR );

//  Img_GuidedFilter( mGuide, mSrc, mDst, r, eps, -1);

    class GuidedFilter gf( mGray, r, eps );

    Mat mFilted = gf.filter(mSrc, -1);
    mFilted.copyTo(mDst);
}

void dehazing::myGuidedFilterGray(Mat &mGuide, int r, float eps)
{
    int height = m_nHei;
    int width = m_nWid;

    /* Init Guidance image and Filtering input image */
    Mat mGuide_32FC1;
    Mat mGray;
    cvtColor( mGuide, mGray, CV_BGR2GRAY );
    mGray.convertTo( mGuide_32FC1, CV_32FC1 );

    Mat mInput_32FC1( height, width , CV_32FC1, m_pfTransmission );

    //N = boxfilter(ones(hei, wid), r); % the size of each local patch; N=(2r+1)^2 except for boundary pixels.
    Mat N;
    boxFilter( Mat::ones(height, width, CV_32FC1), N, -1, cv::Size(r, r) );


    //mean_I = boxfilter(I, r) ./ N;
    Mat mean_I;
    boxFilter( mGuide_32FC1, mean_I, -1, cv::Size(r, r) );

    //mean_p = boxfilter(p, r) ./ N;
    Mat mean_p;
    boxFilter( mInput_32FC1, mean_p, -1, cv::Size(r, r) );

    //printf("row1=%d, col1=%d, row1=%d, col1=%d\n", mGuide_32FC1.rows, mGuide_32FC1.cols, mInput_32FC1.rows, mInput_32FC1.cols);
    //
    //mean_Ip = boxfilter(I.*p, r) ./ N;
    Mat mean_Ip;
    boxFilter( mGuide_32FC1.mul(mInput_32FC1), mean_Ip, -1, cv::Size(r, r) );
#if 1
    ...
    Mat mean_II;
    boxFilter( mGuide_32FC1.mul(mGuide_32FC1), mean_II, -21, cv::Size(r, r) );
    ...
    Mat mean_a;
    boxFilter( a, mean_a, -1, cv::Size(r, r) );
    mean_a = mean_a/N;

    //mean_b = boxfilter(b, r) ./ N;
    Mat mean_b;
    boxFilter( b, mean_b, -1, cv::Size(r, r) );
    mean_b = mean_b/N;

    //q = mean_a .* I + mean_b; % Eqn. (8) in the paper;
    Mat q = mean_a.mul(mGuide_32FC1) + mean_b;

    
    Mat mDst( height, width , CV_32FC1, m_pfTransmissionR );
    q.copyTo(mDst);
...
void dehazing::GuidedFilterShiftableWindow(float fEps)
{
	...
	int x1, x2, y1, y2;
	int nItersX, nItersY;
	float fOptVariance; //fVariance, fOptVariance;
...
iterative:
	...
	for(nY=0; nY<m_nHei; nY++)
	{
		...
		{
			//find minimum
			fOptVariance = (float)99999999999999;
